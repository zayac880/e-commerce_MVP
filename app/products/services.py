...
async def update_product(product_id: int, product_data: ProductCreateUpdateSchema):
    product = await Product.filter(id=product_id).first()
    if product:
        await product.update_from_dict(product_data.model_dump(exclude_unset=True))
        await product.save()
        return product
    else:
        return None
